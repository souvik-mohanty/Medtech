# MedTech Lab Technician App

Login: Google OAuth **+** franchise/clinic/store ID — the account must be
...
Auth flow + a dashboard showing an "Assigned Lab Tests" list — but there is
no Lab Test Booking backend module yet, so that list is **static mock
data** (`lib/features/home/home_screen.dart`), not a real API call. It's
there to illustrate the intended UI; swap in a real fetch once that backend
module exists.
