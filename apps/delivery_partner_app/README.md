# MedTech Delivery Partner App

Login: Google OAuth **+** franchise/store ID — the account must be
...
Auth flow + a dashboard showing a "Delivery Queue" list — but there is no
delivery-assignment backend module yet, so that list is **static mock
