# MedTech Customer Support App

Login: Google OAuth only, platform-wide (not franchise-scoped). See
[docs/PROJECT_SPEC.md](../../docs/PROJECT_SPEC.md) for the full login policy
by role.

Auth flow + a dashboard showing a "Support Tickets" list — but there is no
support-ticket backend module yet (that feature was explicitly deferred, not
just unbuilt), so that list is **static mock data**
(`lib/features/home/home_screen.dart`), not a real API call. It's there to
illustrate the intended UI; swap in a real fetch once that backend module
exists.
